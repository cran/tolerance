nlregtol.int=function(formula,xy.data=data.frame(),x.new=NULL,side=1,alpha=0.05,P=0.99,maxiter=50,...){
n=nrow(xy.data)
...
out=try(suppressWarnings(nls(formula=form,data=xy.data,control=list(maxiter=maxiter,warnOnly=TRUE),...)),silent=TRUE)
test.sig=class(try(summary(out)$sigma,silent=TRUE))
maxiter=ceiling(maxiter/2)
if(maxiter<=1) test.sig="quit"
}
...
t.delta=suppressWarnings(qt(1-alpha,df=n-pars,ncp=delta))
t.delta[is.na(t.delta)]=Inf
K=t.delta/sqrt(n.star)
K[is.na(K)]=Inf
upper=y.hat+sigma*K
lower=y.hat-sigma*K
temp=data.frame(cbind(alpha,P,y.hat,xy.data[,1],lower,upper))
colnames(temp)=c("alpha","P","y.hat","y","1-sided.lower","1-sided.upper")
} else{
z.p=qnorm((1+P)/2)
z.a=qnorm((2-alpha)/2)
...
for(i in 1:length(n.star)){
	V=1+z.a^2/n.star[i]+((3-z.p^2)*z.a^4)/(6*n.star[i]^2)
	K.1=suppressWarnings(z.p*sqrt(V*(1+(n.star[i]*V/(2*df))*(1+1/z.a^2))))
	chi.a=qchisq(alpha,df=df)
	K.2=suppressWarnings(z.p*sqrt(((df*(1+1/n.star[i]))/(chi.a))*(1+(df-2-chi.a)/(2*(n.star[i]+1)^2))))
	if((is.na(K.1)==FALSE & is.na(K.2)) | (df>df.cut[i] & is.na(K.1)==FALSE & is.na(K.2)==FALSE)){
	V=1+z.a^2/n.star[i]+((3-z.p^2)*z.a^4)/(6*n.star[i]^2)
	K[i]=z.p*sqrt(V*(1+(n.star[i]*V/(2*df))*(1+1/z.a^2)))
	} else{
	chi.a=qchisq(alpha,df=df)
	K[i]=z.p*sqrt(((df*(1+1/n.star[i]))/(chi.a))*(1+(df-2-chi.a)/(2*(n.star[i]+1)^2)))
	}
}
upper=y.hat+sigma*K
lower=y.hat-sigma*K
temp=data.frame(cbind(alpha,P,y.hat,xy.data[,1],lower,upper))
colnames(temp)=c("alpha","P","y.hat","y","2-sided.lower","2-sided.upper")
}
