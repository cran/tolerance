nptol.int=function(x,alpha=0.05,P=0.99,side=1,method=c("WILKS","WALD","HM"),upper=NULL,lower=NULL){
n=length(x)
x.sort=sort(x)
...
if(side==2){
if(floor((n+1)/2)==((n+1)/2)) up=((n+1)/2)-1 else up=floor((n+1)/2)
r=1:up
out2=pbeta(P,n-2*r+1,2*r,lower.tail=FALSE)-(1-alpha)
ti2=cbind(r,out2)
...
mins2=min(temp2[,2])
temp2=matrix(temp2[temp2[,2]==mins2,],ncol=2)
r=temp2[,1]
lower=x.sort[r]
upper=x.sort[n-r+1]
}
...
s=n-r+1
if(r<1){
lower=lower} else lower=x.sort[r]
if(s>n){
upper=upper} else upper=x.sort[s]
...
out3=pbeta(P,t-s,n-t+s+1,lower.tail=FALSE)-(1-alpha)
ti3=cbind(s,t,out3)
...
if(side==2){
if(diff==0){
if(lower) x.sort=c(lower,x.sort)
if(upper) x.sort=c(x.sort,upper)
...
temp=data.frame(cbind(alpha,P,x.sort[HM[,1]],x.sort[HM[,2]]))
colnames(temp)=c("alpha","P","2-sided.lower","2-sided.upper")
} else{
l=0:n
u=1:(n+1)
low.temp=cbind(l,((1-pbinom(l-1,n,1-P))-(1-alpha)))
l=matrix(low.temp[low.temp[,2]>0,],ncol=2)
l=l[which.max(l[,1]),][1]
if(l>0) lower=x.sort[l]
up.temp=cbind(u,(pbinom(u-1,n,P))-(1-alpha))
u=matrix(up.temp[up.temp[,2]>0,],ncol=2)
u=u[which.min(u[,1]),][1]
if(u<(n+1)) upper=x.sort[u]
temp=data.frame(cbind(alpha,P,lower,upper))
colnames(temp)=c("alpha","P","1-sided.lower","1-sided.upper")
}}
temp
