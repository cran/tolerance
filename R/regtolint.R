...
if(is.vector(new.x)) new.x=matrix(new.x,ncol=1)
n=length(reg$res)
...
colnames(temp)=c("alpha","P","y","y.hat","1-sided.lower","1-sided.upper")
} else{
z.p=qnorm((1+P)/2)
z.a=qnorm((2-alpha)/2)
...
lower=y.hat-sqrt(MSE)*K
temp=data.frame(cbind(alpha,P,y,y.hat,lower,upper))
colnames(temp)=c("alpha","P","y","y.hat","2-sided.lower","2-sided.upper")
}
